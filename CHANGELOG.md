...
Management API tools now come from `@neon/tools` 0.5.0. Hosts select SDK method paths (`projects.list`); published names are those paths in snake_case (`projects_list`). `create_project`, `create_branch`, and `delete_branch` keep those names. Operation-backed writes wait. Raw Management API operations, `operations.waitFor`, compute-less creates, JWKS, VPC, anonymization, masking, schema compare, and org-member reads are not tools. Host tools stay for SQL, inspect, migrations, query tuning, docs, search, `list_organizations`, `get_connection_string`, and redacted `get_neon_auth_config`. New `?category=` values: `functions`, `storage`, `endpoints`, `snapshots`. `?category=branches` no longer includes compute-endpoint tools; those are `?category=endpoints`. Unknown `?category=` values are ignored and named on initialize notices. `limit` on a list tool caps how many items come back.
