...
Companion to `ai-notes/refresh-slo.md`. Tracks the **initial authorization** flow: every time an MCP client (Cursor, Claude, ChatGPT, etc.) sends a user through `/api/authorize` → upstream Hydra → back to our `/callback` → handed off to the client's redirect URI.
...
Same pattern as `ai-notes/refresh-slo.md` — pull per-bucket JSONL via the Vercel CLI, then aggregate.
