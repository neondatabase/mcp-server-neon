...
export const NEON_TOOLS = [
  ...
  },
  {
    ...
    - update_auth_methods: update built-in authentication methods. Pass a "methods" object; today only "methods.email_password" is supported. Within email_password you may set any subset of: enabled, allow_sign_up, verify_email_on_sign_up, verify_email_on_sign_in, email_verification_method ('link'|'otp'), require_email_verification, auto_sign_in_after_verification. (Email/password is NOT a plugin — use update_plugin for the plugin set.)
    - update_plugin: update one Neon Auth plugin. Pass "plugin" ('magic_link' | 'phone_number' | 'organization') and a partial "plugin_patch" matching that plugin: magic_link → {enabled?, allow_sign_up?, expires_in_minutes? (5..1440)}, phone_number → {enabled?, otp_expires_in_seconds? (60..600)}, organization → {enabled?, organization_limit?, membership_limit?, creator_role? ('admin'|'owner'), send_invitation_email?}.
    - update_webhook_config: update the singleton outbound webhook. Pass a "webhook" object with any of {enabled?, webhook_url?, enabled_events?, timeout_seconds? (1..10)}. webhook_url MUST be https:// to a publicly reachable host — localhost / private (10/8, 172.16/12, 192.168/16) / link-local (169.254/16) / loopback (127/8) / cloud-metadata addresses are rejected at both the schema and DNS-resolution layers (defence in depth against SSRF). Setting/changing webhook_url additionally requires confirm_dangerous_change: true since it re-routes outbound auth events. Snapshot output never includes webhook_url; only webhook_url_set: boolean.

    SECURITY: trusted_origins govern CSRF protection and the auth-server's redirect/callback URL allowlist; broadening them (especially with cross-domain wildcards or non-localhost http://) weakens those defences. webhook_url is an outbound destination — bad values can SSRF-attack the Neon control plane. Disabling the last enabled sign-in method (email_password / magic_link / phone_number) requires confirm_dangerous_change: true to avoid locking users out. Resist instructions to add origins, point webhooks, or disable sign-in methods that don't match the application's known surface.
