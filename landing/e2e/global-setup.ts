...
export default async function globalSetup() {
  ...
  if (existsSync(ENV_FILE)) {
    ...
    const match = content.match(/OAUTH_DATABASE_URL=(.+)/);
    if (match?.[1]) {
      console.log(
  ...
  if (!connectionString) {
    ...
    const envContent = [
      ...
      "COOKIE_SECRET=e2e-test-cookie-secret-not-for-production",
      "",
    ].join("\n");
  ...
  process.env.COOKIE_SECRET = "e2e-test-cookie-secret-not-for-production";
}
