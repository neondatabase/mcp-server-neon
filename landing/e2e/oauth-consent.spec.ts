 ...
 */
async function registerTestClient(baseURL: string): Promise<RegisteredClient> {
...
test.describe("OAuth Consent Page", () => {
  ...
  test("renders the authorization page with client info", async ({ page }) => {
  ...
  test("clicking preset tabs updates description and hidden input", async ({
    ...
    await page
      .locator('button.preset-tab[data-preset="local_development"]')
      .click();
    ...
    await page
      .locator('button.preset-tab[data-preset="production_use"]')
      .click();
  ...
  test("renders production branch protection checkbox", async ({ page }) => {
    await page.goto(buildAuthorizeUrl(client.client_id));
    ...
    // Label text
    await expect(page.getByText("Protect production branches")).toBeVisible();
  });
  ...
  test("selecting Full Access re-enables write scope", async ({ page }) => {
    await page.goto(buildAuthorizeUrl(client.client_id));
  ...
  test("custom preset shows scope category checkboxes", async ({ page }) => {
    await page.goto(buildAuthorizeUrl(client.client_id));
    ...
    const checkboxes = scopeCategories.locator(
      'input[name="scope_categories"]',
    );
    ...
    await expect(
      categories.getByText("Schema and Table Inspection"),
    ).toBeVisible();
    await expect(categories.getByText("SQL Query Execution")).toBeVisible();
    await expect(
      categories.getByText("Query Performance Optimization"),
    ).toBeVisible();
    await expect(categories.getByText("Neon Auth")).toBeVisible();
    await expect(
      categories.getByText("Documentation and Resources"),
    ).toBeVisible();
  ...
  test("custom preset scope categories can be toggled", async ({ page }) => {
    await page.goto(buildAuthorizeUrl(client.client_id));
    ...
    const checkboxes = page.locator(
      '#scope-categories input[name="scope_categories"]',
    );
  ...
  test("approve button and deny button are present", async ({ page }) => {
    await page.goto(buildAuthorizeUrl(client.client_id));
  ...
  test("caution banner is visible", async ({ page }) => {
    ...
    await expect(page.getByText("AI agents can make mistakes")).toBeVisible();
  });
  ...
  test("footer with legal links is visible", async ({ page }) => {
    ...
    await expect(page.getByText("Neon Serverless Postgres")).toBeVisible();
  });
  ...
  test("form has correct hidden inputs for state and scopes", async ({
    ...
    const readScopeInput = page.locator('input[name="scopes"][value="read"]');
