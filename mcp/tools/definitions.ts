...
export const NEON_TOOLS = [
  ...
  },
  {
    ...
    <important_notes>
      ...
      Omit \`databaseName\` to run a database-scoped check against every database on the branch. The result adds a \`database\` column. \`stalled-queries\`, \`lfc-hit-rate\`, \`working-set\`, and \`replication-slots\` are compute-wide: they run once against the first listed database. Passing \`databaseName\` on those checks only chooses the connection; the result does not include \`databaseName\`. For \`lfc-hit-rate\` and \`working-set\`, cache counters reset when the compute restarts. One failing database fails the whole run. \`bloat\` is a statistical estimate, not a measurement.
