...
export const NEON_TOOLS = [
  ...
  },
  {
    ...
    - send_test_email: dispatch a test message through the branch's already-saved email provider. Pass "test_email" with recipient_email only. The stored SMTP settings and password are used server-side, so the caller does not re-supply the password. Does not mutate the saved email_provider config. A shared provider, a missing configuration, or a non-Better-Auth integration is rejected by the API.
